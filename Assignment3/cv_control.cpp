...
bool findCircleFeature(Mat& img, Mat &backproject, Circle& crcl)
{
    ...
    try {
	//Identify the circles using hough transform
        cv::HoughCircles(backproject, circles, CV_HOUGH_GRADIENT,1 , backproject.rows/8, 100, 50, 0, 0);
        double r = 0;
...
void getImageJacobianCFToFF(PrMatrix3 &Jv, float u, float v, float z, float f, float diameter)
{
  ...
  //For the implementation we use diameter instead of radius everywhere, because the Hough-transform outputs that value
  ...
  Im_Jacobian[1][1] = - f / z;
  Im_Jacobian[1][2] = v / z;
  Im_Jacobian[2][2] = diameter * f / (z * z); //
...
float estimateCircleDepth(float f, float diameter, Circle &crcl)
{
    ...
    float depth;
    /*z = X_x *f / S_u from optics
      z - depth, X_x - real radius of the circle, f -  focal length
      S_u - radius of the circle as projected on the image frame*/

    depth = (diameter / 2) * f / crcl.radius;  
    return depth;
}
...
void transformFromOpenCVFToFF(PrVector3 vector_opencvf, PrVector3& vector_ff) 
{
    ...
    // Convert the Image features obtained in the Find cricle function to feature frame by means of translation
...
void transformVelocityFromCFToEEF(PrVector3 vector_cf, PrVector3& vector_eef)
{
  ...
  as seen on the assigment setup figure. The x axis on the end effector frame is x axis of the camera frame and y axis of the end
  effector frame is the x axis of the camera frame*/
...
void transformVelocityFromEEFToBF(PrVector x_current_bf, PrVector3 vector_eef, PrVector3& vector_bf)
{
  //IMPLEMENT THIS
    ...
    */
    ...
    PrMatrix3 state_rotation = state_quat.matrix();
...
void controlRobot(Circle& crcl, PrVector &x, Mat& img, char *cmdbuf)
{
    ...
    //Calculating the (absolute value of) the radius around the BF
    double radius = sqrt(pow(desired_ee_pose_bf[0],2) + pow(desired_ee_pose_bf[1],2) + pow(desired_ee_pose_bf[2],2));
    //Command the robot to go to the new desired position if its inside the radius
    if(radius > reachable_radius){
